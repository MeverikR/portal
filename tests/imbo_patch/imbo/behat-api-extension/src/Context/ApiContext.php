...
class ApiContext implements ApiClientAwareContext, ArrayContainsComparatorAwareContext, SnippetAcceptingContext {
     ...
     * @Given такое тело запроса:
     * @Given тело запроса:
     * @Given установлено тело запроса:
     * @Given тело запроса будет:
     * @Given тело запроса будет такое:
     * @Given в запросе будет:
     */
     ...
     *
     * @When I request :path
     * @When я запрашиваю :path
     * @When я выполняю запрос :path
     * @When я выполняю запрос на URL :path
     * @When I request :path using HTTP :method
     * @When я запрашиваю :path используя HTTP :method
     * @When я выполняю HTTP :method запрос на :path
     * @When я выполняю HTTP :method запрос :path
     * @When я выполняю HTTP :method запрос на URL :path
     */
    public function requestPath($path, $method = null) {
        if (preg_match('/<<([a-z]\w*)>>/i', $path, $regs)) {
            $a_name = $regs[1];
            $__path = getcwd() . DIRECTORY_SEPARATOR ;
            $fn = '___json_key_storage___.tmp';

            if (\file_exists($__path . $fn)){
                $storage = json_decode(file_get_contents($__path . $fn), true) ;
                if (isset($storage[$a_name])){
                    $path = preg_replace('/<<([a-z]\w*)>>/i', $storage[$a_name], $path);
                }
    ...
    /**
     * Удаление пользователя из предыдущего сценария
     *
     * @param string $path The path to request
     * @param string $method The HTTP method to use
     * @return self
     *
     * @When я удаляю клиента из предыдущего сценария
     */
    public function iaDeleteUserFromLastQuery() {

        $fn = getcwd() . DIRECTORY_SEPARATOR . '__user__.tmp';
        $uid = file_get_contents($fn);
        $this->setRequestPath('/clients/' . $uid );
        $this->setRequestMethod('DELETE', false);
        unlink($fn );
        return $this->sendRequest();
    }
    ...
    /**
     * Обновление пользователя из предыдущего сценария
     *
     * @param string $path The path to request
     * @param string $method The HTTP method to use
     * @return self
     *
     * @When я обновляю ранее созданного клиента
     */
    public function iaUpdateUserFromLastQuery() {

        $fn = getcwd() . DIRECTORY_SEPARATOR . '__user__.tmp';
        $uid = file_get_contents($fn);
        $this->setRequestPath('/clients/' . $uid );
        $this->setRequestMethod('PUT', false);
     ...
     * @Then код ответа :code
     * @Then код ответа сервера :code
     * @Then получаем код ответа :code
     * @Then получаю код ответа :code
     * @Then получаем код ответа сервера :code
     * @Then получаю код ответа сервера :code
     * @Then получаем код ответа API :code
     * @Then получаю код ответа API :code
     * @Then код ответа API :code
     */
     ...
     * @Then причина в ответе соответствует условию :expression
     */
     ...
     * @Then в ответе присутствует JSON массив размером как минимум :length элементов
     * @Then в ответе присутствует JSON массив длинной как минимум :length элементов
     * @Then в ответе присутствует JSON массив размером как минимум :length элемента
     * @Then в ответе присутствует JSON массив размером как минимум :length элемент
     * @Then в ответе присутствует JSON массив длинной как минимум :length элемента
     * @Then в ответе присутствует JSON массив длинной как минимум :length элемент
     *
     */
     ...
     * @Then в ответе содержится следующий JSON:
     * @Then в ответе содержится такой JSON:
     * @Then в ответе содержится примерно такой JSON:
     * @Then в ответе есть такой JSON:
     * @Then в ответе получаю такой JSON:
     * @Then в ответе есть примерно такой JSON:
     */
    ...
    /**
     * Сохранить свойство из JSON (только первого уровня) под каким-то ключем
     *
     * @param string $param какой ключ брать на 1м уровне JSON объекта
     * @param string $name под каким именем сохранить
     * @return void
     *
     * @Then сохранить из ответа JSON значение свойства :param как :name
     */
    public function saveJsonKeyAsName($param, $name) {
        $this->requireResponse();
        ...
        if (\file_exists($path . $fn)){
            $storage = json_decode(file_get_contents($path . $fn), true) ;
        } else {
            $storage = [];
        }
        ...
        file_put_contents($path . $fn, $storage);
    ...
    /**
     * Сохранить ID-шник пользователя из тела ответа
     *
     * @param PyStringNode $contains
     * @throws AssertionFailedException
     * @return void
     *
     * @Then я проверил и сохранил юзера из ответа, должны быть такие данные:
     */
    public function iaProverilISokhranilIuzeraIzOtvetaDolzhnyBytTakieDannye(PyStringNode $contains) {
        $this->requireResponse();
    ...
    /**
     * Удалить всякие временные файлы
     *
     * @return void
     *
     * @Then почистить временные файлы
     * @Then удалить временные файлы
     * @Then прибраться
     */
